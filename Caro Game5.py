    ...
    if max_dgr.__len__() < 3:
        ...
        try:
            if state[border_head[0]][border_head[1]] == '.' and state[border_tail[0]][border_tail[1]] == '.':
                return - table_dgr[max_dgr.__len__()]*2
...
#                 print(max_dgr)
                return table_dgr[max_dgr.__len__()]*4
            else:
                return 0
        except:
            return table_dgr[max_dgr.__len__()]
    ...
    for src in ls_opp_point[:-1]:
        for des in ls_opp_point[ls_opp_point.index(src) + 1:]:
            if near(src, des):
                ...
                elif less_dangerous(state, max_dgr, turn) < less_dangerous(state, level_dgr, turn) and not is_in_ls_set(level_dgr, ban_line):
                    max_dgr = level_dgr
                if potential_point < potential(state, level_dgr):
                    potential_point = potential(state, level_dgr)
                if level_dgr.__len__() >= 3 and less_dangerous(state, level_dgr, turn) > 0:
                    make_defend += less_dangerous(state, level_dgr, turn)
#     print(max_dgr)
#     print(make_defend)
    return less_dangerous(state, max_dgr, turn) + table_dgr[min(potential_point, 5)] + make_defend - ban_line.__len__()*5 #+ drop_plot(state, ls_opp_point)
...
#     print('makelike: ' + str(score_make_line))
#     print('defend: ' + str(score_in_opp_line))
...
def load_game():
    num_plot = size   #int(input('Number x Number plot: '))
